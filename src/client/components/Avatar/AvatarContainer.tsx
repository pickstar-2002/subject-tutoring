...
export const AvatarContainer: React.FC<AvatarContainerProps> = ({
  ...
  const handleConnect = async () => {
    ...
    const controller = new AvatarController({
      ...
      onError: (error) => {
        ...
        setInitError(error?.message || '连接失败')
  ...
  return (
    <div className="relative h-full min-h-0 flex flex-col">
      ...
      {/* 状态指示栏 */}
      <div className="bg-white px-4 py-3 rounded-b-2xl border-t flex justify-between items-center shadow-sm flex-shrink-0">
        ...
        <div className="flex items-center space-x-3">
          ...
          {!isInitialized && !isConnecting ? (
            <button
              ...
              className="px-3 py-1.5 bg-blue-500 text-white text-xs font-medium rounded-lg hover:bg-blue-600 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1"
            >
              <span>🔗</span>
              <span>连接</span>
            </button>
          ...
          <div className="text-xs text-gray-400 flex items-center space-x-1">
            <span className="hidden sm:inline">魔珐星云驱动</span>
            {isInitialized && !isConnecting && !isDisconnecting && <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />}
          </div>
        </div>
      </div>
    </div>
...
const StateIndicator: React.FC<{ state: AvatarState; isInitialized: boolean; hasError: boolean }> = ({ state, isInitialized, hasError }) => {
  let colorClass = AVATAR_STATE_COLORS[state] || 'bg-gray-400'
